...
type tAdminTechDialogEditProps = {
  open: boolean,
  handleClose: () => void,
  tech: tTech | null
}

const AdminTechDialogEdit = memo(({ open, handleClose, tech: tch }: tAdminTechDialogEditProps) => {
  ...
  const handleSave = useCallback(async () => {
    try {
      await API.put(`/techs/${tch?._id}`, tech, { withCredentials: true });
      handleClose();
      setAlert({ severity: 'success', children: "The tech has been edited successfully."})
    } catch (err) {
  ...
  return (
    ...
    >
      <DialogTitle variant="h5">Edit Tech</DialogTitle>
      <Divider />
      <DialogContent>
...
export default AdminTechDialogEdit;