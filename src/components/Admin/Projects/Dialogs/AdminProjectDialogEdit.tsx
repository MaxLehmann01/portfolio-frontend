...
type tAdminProjectDialogEditProps = {
  open: boolean,
  handleClose: () => void,
  project: tProject | null
}

const AdminProjectDialogEdit = memo(({ open, handleClose, project: prjct }: tAdminProjectDialogEditProps) => {
  ...
  const handleSave = useCallback(async () => {
    try {
      await API.put(`/projects/${prjct?._id}`, project, { withCredentials: true });
      handleClose();
      setAlert({ severity: 'success', children: "The project has been edited successfully."})
    } catch (err) {
  ...
  }, [project, handleClose, setAlert, prjct]);
  ...
  useEffect(() => {
    if(open) fetchTechs();
    if(prjct) setProject(prjct);
  }, [open, fetchTechs, prjct])
...
export default AdminProjectDialogEdit;